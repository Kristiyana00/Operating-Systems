...
{
        if (argc != 7){
                errx(1, "Usage: %s <6 file names>", argv[0]);
        }
    ...
    if ((r_status = read(fd5, suffix_arr, suffix_c * sizeof(uint64_t))) != (ssize_t)(suffix_c * sizeof(uint64_t)))
    {
        int olderrno = errno;
        close(fd1);
        close(fd2);
